##' Train the cell-type composition predictor using PLS regression on methylation dataset 
##'
##' @title Train a predictor for cell-type composition
##' @param data Matrix with DNAm beta-values
...
train_blood_cell <- function(data, covar, cellPer, model=formula(cellPer ~ covar + data), 
...
##' @param pred A matrix or mvr predictor, trained using train_blood_cell
...
predict_blood_cell <- function(pred, data, covar, transformation=function(x) x, ncomp=NULL, impute=TRUE, ...) {
...
##' Plots to validate predictor a predictor trained using the train_blood_cell function
##'
...
plot_blood_cell <- function(measured, predicted, ...)
{ 
